...
GVRPSolver::GVRPSolver(Instance* instance){
	...
	solDir = OUTPUT_DIRECTORY + curInstance->name + "/";
	solQuality = 0;
	numVehicle = 0;
	numVisitedClients = 0;
	executionTime = 0; 
	...
	/*TESTING*/
	cout << "RUNNING TEST FOR INSTANCE " << curInstance->name << " AND CONFIG ";
	cout << "numIter = " << curInstance->iter << " lenTabu = " << curInstance->lentabu << "\n";
...
void GVRPSolver::writeSolution() {
	...
	string n = solDir + curInstance->name + "[" + to_string(curInstance->iter) + "," + to_string(curInstance->lentabu) + "]";
	ofstream out(n + ".out");
	if(out.is_open()) {
		out << numVisitedClients << "\n";
		out << numVehicle << "\n";
		out << curInstance->numCustomers << "\n";
		out << curInstance->numStations << "\n";
		out << greedyQuality << "\n";
		out << solQuality << "\n";
		out << executionTime << "\n";
...
vehicleSolution GVRPSolver::tabuSearch(vehicleSolution greedySol) {
	...
	size_t sizeTabu = curInstance->lentabu;
	swapPair movement;
	swapPair best_movement;

	double bestNeighborQuality;

	int t = curInstance->iter;
	while(t--) {
